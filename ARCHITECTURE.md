...
2. **The database is the only thing that's hard to scale. Everything else is stateless.** API containers and workers scale horizontally by turning a dial. Postgres does not. So we spend our up-front design effort on the data model and its indexes, and keep every other component free of local state.
...
- **`journeys` / `journey_segments`** — a journey is what the user bought; a segment is one train (multi-leg journeys matter for missed connections later). Each segment stores the matched service identity (RID/UID from Darwin) — matching happens once at creation, so the hot ingest path is a pure lookup. **Not partitioned — decided September 2026.** The original plan was monthly partitions on `travel_date`. Postgres requires the partition key in every unique constraint on a partitioned table and in every foreign key that points at it, and two tables (`delay_detections`, `claims`) already reference `journeys(id)` and rely on it being unique on its own. Partitioning later would mean adding `travel_date` to both, rebuilding their constraints and rewriting the queries on them, and every new table pointing at journeys makes that bigger. Claims are only valid 28 days back, so nobody queries old months except for audit, and an index on `travel_date` carries tens of millions of rows without trouble. If the table must ever shrink, archive the journeys of long-inactive users — per user, not per month.
...
- **Trust at the door** (`journeys.intake.screen`; migrations 0013, 0015 and 0016). A ticket email reaches us two ways, and they prove opposite things. **Manual**: the person presses Forward, so their provider composes a new message signed as them — strong proof of *who*, weak proof of *what*, because the retailer's original headers are now editable text in the body. **Automatic**: a forwarding rule redirects the retailer's message untouched, so the From header still names the retailer and the retailer's own DKIM signature still validates — strong proof of *what*, none at all of *who*, because the only headers naming the forwarding account are added after signing and any sender can type them. An earlier version of the door required From to equal the account email; that is right for the manual shape and rejects every message of the automatic shape, which is the one a person sets up once and forgets. So the door does not judge identity from the message. It records which shape arrived (`inbound_emails.forwarding`) and refuses only what it can actually judge: an explicit SPF or DKIM failure from the mail provider, and a per-user cap of 50 emails per rolling day, which bounds what a leaked address can spend on the reader. Refused mail is kept without its body, with the reason shown to the user.
- **The forwarding address is the credential**, so it is minted at 128 bits and the user can replace it (`POST /intake/address/rotate`). It is not a secret that stays hidden: it appears in the headers of every message that touches it and in the user's own mail settings, so it is sized against guessing and designed to be rotated after a leak. Identity is re-established where it can be: nothing is filed until the user confirms the parsed journey in the app.
- **Gmail's setup message** (`journeys.intake.forwarding_confirmation`) is recognised and filed as `confirmation` with its code in `status_reason`, never handed to the reader. Gmail will not begin forwarding until a code it emails to the destination is typed back into Gmail; answered "not a ticket" and filed away, the user could never switch forwarding on at all. Binding that requesting Gmail account to the user at enrolment is the next step and is not built.
- **What the reader is given**: the body's words, HTML stripped, cut at 50,000 characters. Bodies are blanked 60 days after the email was decided; the structured reading stays.
- **Delivery** (decided September 2026): Cloudflare Email Routing, free, with a catch-all on a subdomain so every `tickets-<code>@` is accepted without registering anything per user. Postmark charges for inbound and cannot send a custom header on its inbound webhook; SES has no HTTPS action at all and needs SNS or S3 in between. Neither is worth it here. The Cloudflare Email Worker either calls the API through a free tunnel or stores the message for the scheduler to collect, so no public address is required. Note that Email Workers currently receive no usable `Authentication-Results`, so `spf_pass` and `dkim_pass` stay null on that path and the door's provider check simply does not fire.
...
| **3 — Write pressure & table bloat** (~200k users) | Write IOPS climbing; table sizes large; vacuum struggles | Millions of journey rows; delay-event writes contending with app traffic | Bigger instance (vertical scaling is unfashionable and *correct* — buy years for money before buying complexity). The `travel_date` indexes keep the working set to recent rows (§4 says why the table is not partitioned); if the table itself must shrink, archive the journeys of long-inactive users to S3 — per user, not per month. |
