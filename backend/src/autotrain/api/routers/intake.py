...
Four routes, two audiences. The mail provider's webhook posts each email
...
the line. The signed-in user reads their own forwarding address, replaces it
when it has leaked, and sees what became of the emails they sent to it.

Thin by rule (ARCHITECTURE §3), and a composition point like
routers/journeys.py: identity knows which account an address belongs to;
journeys owns the emails, the door they pass through and what is read out of
them. The webhook only STORES — reading is the scheduler's intake sweep — so
the provider gets its 202 in milliseconds and a slow reader can never make
it retry.
"""
...
def _inbound_domain() -> str:
    """The domain forwarding addresses live on, or a 503. Blank counts as
    unset, as with app_base_url: an address with no domain would be shown
    to the user and bounce."""
...
def receive_email(
    ...
    secret checks out: a provider retries on 4xx/5xx, and neither a repeat
    delivery nor anything the door refuses (an unknown recipient, a sender
    that is not the account, a failed provider check, the daily cap) is
    something a retry would fix. The receipt says which it was."""
    _require_intake_secret(x_autotrain_intake_secret)
    owner = identity.user_by_forwarding_address(conn, payload.recipient)
    stored = journeys.receive_ticket_email(
        ...
        daily_cap=get_settings().intake_daily_cap,
    )
    if stored is None:
        return IntakeReceipt(status="duplicate")
    return IntakeReceipt(status="rejected" if stored.status == "rejected" else "received")
...
@router.post("/address/rotate")
def rotate_forwarding_address(conn: ConnDep, user_id: UserIdDep) -> ForwardingAddressOut:
    """A fresh address in place of the old, for one that has leaked or is
    being spammed. Mail to the old address is nobody's from here on; the
    user updates their forwarding rule and nothing else changes."""
    domain = _inbound_domain()
    code = identity.rotate_forwarding_code(conn, user_id)
    if code is None:
        raise HTTPException(status_code=404, detail="unknown user")
    return ForwardingAddressOut(address=identity.forwarding_address(code, domain))
